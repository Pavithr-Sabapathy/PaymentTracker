...
import com.mashreq.paymentTracker.serviceImpl.UAEFTSReportServiceImpl;
import com.mashreq.paymentTracker.type.CountryType;

@SpringBootTest
@AutoConfigureMockMvc
public class UAEFTSReportServiceTest {
	...
	@Autowired
	UAEFTSReportServiceImpl UAEFTSReportServiceImpl;

	@MockBean
	private LinkReportService linkReportService;

	@Test
	void testProcessAdvanceSearchReport() {
		...
		List<AdvanceSearchReportOutput> actualProcessFlexReportResult = UAEFTSReportServiceImpl
				.processAdvanceSearchReport(advanceSearchReportInput, mockComponentsList, reportContext);
	...
	void testProcessReport() {
		...
		mockFederatedReportPromptDTO.setPromptKey("uaefts-ccn");
		...
		mockUAEFTSReportOutputList.add(reportOutput);
	...
	@Test
	void testPopulateDataForAdvanceSearch() {
		ReportInstanceDTO reportInstance = new ReportInstanceDTO();
		reportInstance
				.setCreationDate(Date.from(LocalDate.of(1970, 1, 1).atStartOfDay().atZone(ZoneOffset.UTC).toInstant()));
		reportInstance.setId(1L);
		...
		ReportInput response = UAEFTSReportServiceImpl.populateBaseInputContext(reportContext);
