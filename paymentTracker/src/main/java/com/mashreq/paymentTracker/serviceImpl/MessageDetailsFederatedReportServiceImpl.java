...
import com.mashreq.paymentTracker.service.MessageDetailsFederatedReportService;
...
public class MessageDetailsFederatedReportServiceImpl implements MessageDetailsFederatedReportService {
	...
	public ReportExecuteResponseData processMessageDetailReport(ReportInstanceDTO reportInstanceDTO,
		...
		FederatedReportQueryData federatedReportQueryData = new FederatedReportQueryData();
			...
			if (!componentList.isEmpty()) {
				for (Components component : componentList) {
					...
					if (null != reportComponent.getActive() && reportComponent.getActive().equals(CheckType.YES)) {
						...
						messageDetailsFederatedReportInput = populateBaseInputContext(
								reportInstanceDTO.getPromptsList());
						List<SWIFTMessageDetailsFederatedReportOutput> messageDetails = processComponents(
								messageDetailsFederatedReportInput, reportContext, reportComponent,
								federatedReportQueryData, reportInstanceDTO);
						messageData = populateSwiftDetailedReportData(messageDetails);
...
//	}
	...
	private List<SWIFTMessageDetailsFederatedReportOutput> processComponents(
			MessageDetailsFederatedReportInput reportInputContext, ReportContext reportContext,
			ReportComponentDTO reportInstanceComponentsDTO, FederatedReportQueryData federatedReportQueryData,
			ReportInstanceDTO reportInstanceDTO) {
		List<SWIFTMessageDetailsFederatedReportOutput> out = new ArrayList<SWIFTMessageDetailsFederatedReportOutput>();
		if (null != reportInstanceComponentsDTO) {
			...
			if (messageThrough.equalsIgnoreCase(MashreqFederatedReportConstants.MESSAGE_THROUGH_SWIFT)) {
				processSwiftMessage(reportInputContext, reportContext, reportInstanceComponentsDTO,
						federatedReportQueryData, reportInstanceDTO);
			} else if (messageThrough.equalsIgnoreCase(MashreqFederatedReportConstants.MESSAGE_THROUGH_UAEFTS)) {
				processUaeftsMessage(reportInputContext, reportContext, reportInstanceComponentsDTO,
						federatedReportQueryData, reportInstanceDTO);
			}
		}
		return out;
	...
	private void processSwiftMessage(MessageDetailsFederatedReportInput reportInputContext, ReportContext reportContext,
		...
		ReportComponentDTO instanceComponent = getMatchedInstanceComponent(reportInstanceComponentsDTO,
				MashreqFederatedReportConstants.MESSAGE_DETAILS_SWIFT_COMPONENT_KEY);
		if (instanceComponent != null) {
			reportInputContext.setComponent(instanceComponent);
			List<SWIFTMessageDetailsFederatedReportOutput> reportComponentData = swiftDetailedReportService
					.processMessageDetailsReport(reportInputContext, reportInstanceComponentsDTO, reportContext);
...
//						.getCannedReportInstanceMetrics(reportInstanceDTO.getId()));
	...
	public FederatedReportOutput createFederatedDefaultOutput(SWIFTMessageDetailsFederatedReportOutput baseOutput) {
		FederatedReportOutput output = new FederatedReportOutput();
		output.setComponentDetailId(baseOutput.getComponentDetailId());
		return output;
	}

	public ReportComponentDTO getMatchedInstanceComponent(ReportComponentDTO component, String compKey) {
		ReportComponentDTO matchedComponent = null;
		if (component.getComponentKey().equalsIgnoreCase(compKey) && CheckType.YES == component.getActive()) {
	...
	private void processUaeftsMessage(MessageDetailsFederatedReportInput reportInputContext,
			ReportContext reportContext, ReportComponentDTO cannedReportInstanceComponents,
			FederatedReportQueryData federatedReportQueryData, ReportInstanceDTO cannedReportInstance) {
		ReportComponentDTO instanceComponent = getMatchedInstanceComponent(cannedReportInstanceComponents,
				MashreqFederatedReportConstants.MESSAGE_DETAILS_UAEFTS_COMPONENT_KEY);
		if (instanceComponent != null) {
			reportInputContext.setComponent(instanceComponent);
			//need to check
			List<FederatedReportOutput> reportComponentData = null;// UAEFTSReportService.processUAEFTSReport(reportInputContext, reportContext);
			if (!reportComponentData.isEmpty()) {
				List<FederatedReportOutput> reportOutput = new ArrayList<FederatedReportOutput>();
				if (!reportComponentData.isEmpty()) {
					for (FederatedReportOutput componentOut : reportComponentData) {
						FederatedReportOutput output = (FederatedReportOutput) componentOut;
...
//				 .getCannedReportInstanceMetrics(cannedReportInstance.getId()));
				...
				// important that this is populated before transpose
				 reportOutput = transposeDataElements(reportOutput);
...
//	
	 protected List<FederatedReportOutput> transposeDataElements (List<FederatedReportOutput> dataElements) {
	      List<FederatedReportOutput> outputList = new ArrayList<FederatedReportOutput>();
	      if (dataElements.isEmpty()) {
	         return outputList;
	      }
	      // transpose the data with headers
	      List<List<String>> originalData = new ArrayList<List<String>>();
	      if (!dataElements.isEmpty()) {
	         List<String> columnLabels = dataElements.get(0).getColumnLabels();
	         originalData.add(0, columnLabels);
	      }
	      for (FederatedReportOutput tranpOutput : dataElements) {
	         originalData.add(tranpOutput.getTransformedData());
	      }
	      int numRows = originalData.size();
	      int cols = originalData.get(0).size();
	      for (int index = 0; index < cols; index++) {
	         FederatedReportOutput output = new FederatedReportOutput();
	         List<String> transposedRow = new ArrayList<String>();
	         for (int rowIndex = 0; rowIndex < numRows; rowIndex++) {
	            transposedRow.add(originalData.get(rowIndex).get(index));
	         }
	         output.setTransformedData(transposedRow);
	         outputList.add(output);
	      }
	      return outputList;
	   }
	
	private MessageDetailsFederatedReportInput populateBaseInputContext(List<ReportPromptsInstanceDTO> list) {
